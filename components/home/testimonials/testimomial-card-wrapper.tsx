            ...
            {CardData.map((item) => (
                <div key={item.name} className='bg-white p-10 shadow-sm space-y-6'>
                    ...
                    <div className='flex items-center  gap-3 text-sm'>
                        <Image src={item.imageUrl} alt={item.name} className='rounded-full' width={60} height={60} />
                        <div className='space-y-2'>
                            <p>by<span className='font-semibold ml-2'>{item.name}</span></p>
                            <p className='italic'>{item.position}, {item.company}</p>
                        </div>
                    </div>
                </div>
            ))}
